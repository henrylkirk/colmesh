...
function colmesh() : colmesh_mesh() constructor {
    ...
    static subdivide = function(_regionSize) {
        ...
        colmesh_debug_message(
            ((("colmesh.subdivide: Generated spatial hash with " + string(ds_map_size(spHash))) + " regions in ") +
            string((get_timer() - debugTime) / 1000)) + " milliseconds"
        );
    };
    ...
    /// @function addTrigger(shape, solid*, colFunc*, rayFunc*)
    ...
    /// @function addTriangle(V[9], parent*)
    ...
    #endregion
    /// @function addShapeToSubdiv(shape, regions*, precise*)
    static addShapeToSubdiv = function(shape, regions = undefined, precise = true) {
        ...
        if (is_undefined(regions)) {
            regions = _getRegions(struct.getMinMax());
        }
        ...
        repeat (xNum) {
            ...
            repeat (yNum) {
                ...
                repeat (zNum) {
                    ...
                    if (!precise or struct._intersectsCube((regionSize * 0.5) * 1.001, _x, _y, _z)) { // The 1.001 fixes an issue with triangles placed exactly on the division planes not being added
    ...
    /// @function removeShapeFromSubdiv(shape, regions*)
    static removeShapeFromSubdiv = function(shape, regions = undefined) {
        if (spHash < 0) {
            return false;
        }
        if (is_undefined(regions)) {
            regions = _getRegions(_getShape(shape).getMinMax());
        }
        var xNum = regions[3] - regions[0];
        var yNum = regions[4] - regions[1];
        var zNum = regions[5] - regions[2];
    ...
    static getRegion = function(AABB) {
        ...
        if ((((((minx > maximum[0]) or (miny > maximum[1])) or (minz > maximum[2])) or (maxx < minimum[0])) or
        (maxy < minimum[1])) or (maxz < minimum[2])) {
        ...
        if (spHash < 0) {
            ...
            repeat (i) {
                ...
                if (!_getShape(shape).checkAABB(minx, miny, minz, maxx, maxy, maxz)) {
                    continue;
                } // Only add the shape to the list if its AABB intersects the capsule AABB
                ...
                repeat (zNum) {
                    ...
                    repeat (i) {
                        ...
                        if (ds_list_find_index(tempList, shape) >= 0) {
                            continue;
                        } // Make sure the shape hasn't already been added to the list
                        if (!_getShape(shape).checkAABB(minx, miny, minz, maxx, maxy, maxz)) {
                            continue;
                        } // Only add the shape to the list if its AABB intersects the capsule AABB
                        ds_list_add(tempList, shape);
    ...
    static displaceCapsule = function(
        x,
        y,
        z,
        xup,
        ...
        slopeAngle = 0,
        fast = true,
        executeCollisionFunction = false
    ) {
    ...
    static regionDisplaceCapsule = function(
        region,
        x,
        y,
        z,
    ...
    #region Ray casting
    /// @function castRay(x1, y1, z1, x2, y2, z2, mask*)
    static castRay = function(x1, y1, z1, x2, y2, z2, mask = cmGroupSolid) {
        ...
        while (t < 1) {
            ...
            repeat (i) {
                ...
                if ((mask & shape.group) == 0) {
                    continue;
                } // Continue the loop if the mask does not match the group
                ...
                if (!is_array(hit)) {
                    continue;
                } // There is no intersection between the ray and this shape
                if (hit[7] > t) {
                    continue;
                } // The intersection between the ray and the shape is too far away
                array_push(rayStruct.intersections, hit); // Add shape to intersection list regardless of whether or not it's solid
                if (shape.type ==
                eColMeshShape.Dynamic) { // If this shape is a dynamic, add it to the intersections list as well
    ...
    /// @function regionCastRay(region, ray, mask*, rayStruct*)
    static regionCastRay = function(
        region,
        ray,
        mask = cmGroupSolid,
        rayStruct = new colmesh_raycast_result(ray[3], ray[4], ray[5], 0, 0, 1, false, -1)
    ) {
        ...
        if (is_undefined(region) or (((ray[0] == ray[3]) and (ray[1] == ray[4])) and (ray[2] == ray[5]))) {
            return rayStruct;
        }
    ...
    static _constrain_ray = function(ray) {
        ...
        if ((_min < -1) or (_max > 1)) {
            if (((((((x1 < -1) and (x2 < -1)) or ((y1 < -1) and (y2 < -1))) or ((z1 < -1) and (z2 < -1))) or
            ((x1 > 1) and (x2 > 1))) or ((y1 > 1) and (y2 > 1))) or
            ((z1 > 1) and (z2 > 1))) { // The ray is fully outside the bounding box, and we can end the algorithm here
                return false;
            }
        }
    ...
    /// @function save(path, saveTriggers)
    ...
    static writeToBuffer = function(saveBuff, saveTriggers = false) {
        ...
        for (var i = 0; i < shapeNum; i++) {
            with (_getShape(shapeList[| i])) {
                if (((group & cmGroupSolid) == 0) and !saveTriggers) {
        ...
        colmesh_debug_message(
            ("Script colmesh.writeToBuffer: Wrote colmesh to buffer in " + string(current_time - debugTime)) +
            " milliseconds"
        );
    ...
    static readFromBuffer = function(loadBuff) {
        ...
        colmesh_debug_message(
            ((("Script colmesh.readFromBuffer: Read " + string(self)) + " from buffer in ") +
            string(current_time - debugTime)) + " milliseconds"
        );
        buffer_delete(tempBuff);
        return true;
    };
    ...
    static debugDraw = function(region, tex = -1) {
		...
		*/
        if (is_undefined(region)) { // Exit if the given region is undefined
            exit;
        }
        if (region < 0) {
            region = shapeList;
        ...
        for (var i = 0; i < n; i++) {
            ...
            shader_set_uniform_f(
                shader_get_uniform(sh_colmesh_debug, "u_color"),
