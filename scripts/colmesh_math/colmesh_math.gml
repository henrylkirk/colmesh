...
function colmesh_matrix_build_from_vector(
    ...
    siScale,
    upScale,
    targetM = [abs(vx) < abs(vy), 1, 1, 0, 0, 0, 0, 0, vx, vy, vz, 0, X, Y, Z, 1]
) {
    /*
...
function colmesh_cast_ray_sphere(sx, sy, sz, r, x1, y1, z1, x2, y2, z2, doublesided = false) {
	...
	*/
    ...
    if (t < u) {
        ...
        if (!doublesided) {
            return -1;
        } // The ray started inside the sphere, and the sphere is not doublesided. There is no way this ray can intersect the sphere.
