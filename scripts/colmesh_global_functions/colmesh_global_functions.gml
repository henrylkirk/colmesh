...
function colmesh_block_intersects_AABB(M, minx, miny, minz, maxx, maxy, maxz) {
    ...
    var b = matrix_transform_vertex(I, clamp(M[12], minx, maxx), clamp(M[13], miny, maxy), clamp(M[14], minz, maxz));
    ...
    var bX = (minx + maxx) * 0.5;
    var bY = (miny + maxy) * 0.5;
    var bZ = (minz + maxz) * 0.5;
    var sX = maxx - bX;
    var sY = maxy - bY;
    var sZ = maxz - bZ;
...
function colmesh_load_obj_to_buffer(filename) {
    static read_face = function(faceList, str) {
        ...
        while (i--) {
            for (var j = 2; j >= 0; j--) {
                ...
                } else // If the vertex contains a position and a texture coordinate
                if (string_count("/", vstr) == 1) {
                    v = abs(real(string_copy(vstr, 1, string_pos("/", vstr) - 1)));
                    t = abs(real(string_delete(vstr, 1, string_pos("/", vstr))));
                } else // If the vertex only contains a position
                if (string_count("/", vstr) == 0) {
                    v = abs(real(vstr));
                } else // If the vertex contains a position and normal
                if (string_count("//", vstr) == 1) {
    ...
    if (file == -1) {
        colmesh_debug_message("Failed to load model " + string(filename));
        return -1;
    }
    ...
    while (!file_text_eof(file)) {
        ...
        switch (string_copy(str, 1, string_pos(" ", str) - 1)) {// Load vertex positions

            case "v":
                ds_list_add(V, read_line(str));
                break;// Load vertex normals

            case "vn":
                ds_list_add(N, read_line(str));
                break;// Load vertex texture coordinates
...
/// @function colmesh_convert_2d_to_3d(cameraIndex, x, y)
function colmesh_convert_2d_to_3d(cameraIndex, _x, _y) {
	...
	*/
    ...
    var my = (2 * ((_y / window_get_height()) - 0.5)) / P[5];
