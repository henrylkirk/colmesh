...
function colmesh_raycast_result(x, y, z, nx, ny, nz, hit, struct) constructor {
    ...
    /// @function executeRayFunc(argument0*, argument1*, ..., argument7*)
    static executeRayFunc = function(arg0, arg1, arg2, arg3, arg4, arg5, arg6, arg7) {
		...
		*/
        ...
        repeat (i) {
            ...
            if ((intersection[6].group & cmGroupRayTrigger) == 0) {
                continue;
            }
...
function colmesh_colliders(x, y, z, xup, yup, zup, slopeAngle, precision, mask) constructor {
    ...
    /// @function executeColFunc(argument0*, argument1*, ..., argument7*)
    static executeColFunc = function(arg0, arg1, arg2, arg3, arg4, arg5, arg6, arg7) {
		...
		*/
        ...
        repeat (i) {
            ...
            if ((struct.group & cmGroupColTrigger) == 0) {
                continue;
            }
...
function colmesh_collider_capsule(
    x,
    y,
    z,
    xup,
    ...
    precision = 0,
    mask = 1
) : colmesh_colliders(x, y, z, xup, yup, zup, slopeAngle, precision, mask) constructor {
    self.radius = radius;
    self.height = height;

    /// @function checkForCollision(colMesh)
    static checkForCollision = function(colMesh) {
        // Returns whether or not the given capsule collides with the given colMesh
        var AABB = colmesh_capsule_get_AABB(
            x,
            y,
            z,
            xup,
            yup,
            zup,
            (precision == 0) ? radius : radius * cmFirstPassRadius,
            height
        );
        return checkForCollisionRegion(colMesh, colMesh.getRegion(AABB));
    };

    /// @function checkForCollisionRegion(colMesh, region)
    ...
    /// @function avoidRegion(colMesh, region, mask*)
    static avoidRegion = function(colMesh, region, _mask = self.mask) {
        ...
        if (is_undefined(region) or (cmRecursion >= cmMaxRecursion)) {
        ...
        repeat (i) {
            ...
            if ((_mask & shape.group) == 0) { // If the shape is not in any of the groups in the mask, continue the loop
                continue;
            }
            if ((shape.group & cmGroupSolid) == 0) { // If this shape is not solid, check if there is an intersection
