...
function colmesh_debug_draw_block(M, colour) {
    ...
    shader_set_uniform_f(
        ...
        color_get_red(colour) / 255,
        color_get_green(colour) / 255,
        color_get_blue(colour) / 255,
        1
    );
...
function colmesh_create_capsule(hVerts, vVerts, hRep, vRep) {
    ...
    for (var xx = 0; xx < hVerts; xx++) {
        ...
        for (var yy = 0; yy < vVerts; yy++) {
            var ya1 = ((yy / vVerts) * pi) * 0.5;
            var ya2 = (((yy + 1) / vVerts) * pi) * 0.5;
            var yc1 = cos(ya1);
            var ys1 = sin(ya1);
            var yc2 = cos(ya2);
            var ys2 = sin(ya2);
...
function colmesh_create_cylinder(steps, hRep, vRep) {
    ...
    for (var xx = 0; xx < steps; xx++) {
        ...
        vertex_position_3d(vbuff, xc2, xs2, 0);
        vertex_normal(vbuff, 0, 0, -1);
        vertex_texcoord(vbuff, (0.5 + (0.5 * xc2)) * hRep, (0.5 + (0.5 * xs2)) * vRep);
        vertex_color(vbuff, c_white, 1);
        vertex_position_3d(vbuff, xc1, xs1, 0);
        vertex_normal(vbuff, 0, 0, -1);
        vertex_texcoord(vbuff, (0.5 + (0.5 * xc1)) * hRep, (0.5 + (0.5 * xs1)) * vRep);
        vertex_color(vbuff, c_white, 1);
